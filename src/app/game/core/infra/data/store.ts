class Store {
  ...
  async initialize(): Promise<void> {
    return new Promise<void>(resolve => {
      const req = this.indexedDB.open('spacecraft', 1);
      ...
      req.onupgradeneeded = (event: any) => {
        ...
        db.createObjectStore('entities', { keyPath: 'type', autoIncrement: false });
        db.createObjectStore('backgrounds', { keyPath: 'name', autoIncrement: false });
        db.createObjectStore('effects', { keyPath: 'name', autoIncrement: false });
